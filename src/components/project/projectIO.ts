...
export interface ProjectData {
  ...
  // Photo library (filename -> base64 data URL)
  photoLibrary?: Record<string, string>;
}
...
export const exportProject = async (
  ...
  try {
    ...
    const projectData: ProjectData = {
      ...
      photoLibrary: photoLibrary ? Object.fromEntries(photoLibrary) : undefined,
    };
  ...
  try {
    ...
    const migratedFields = data.fields.map((field: any) => {
      ...
      if (field.type === 'qrcode') {
        return {
          ...field,
          backgroundColor: field.backgroundColor !== undefined ? field.backgroundColor : null,
        };
      }
      if (field.type === 'photo') {
        return {
    ...
    return {
      ...
      photoLibrary: data.photoLibrary ? new Map(Object.entries(data.photoLibrary)) : new Map(),
    };
  } catch (error) {
