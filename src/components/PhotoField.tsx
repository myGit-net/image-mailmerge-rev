...
export interface PhotoFieldData {
  ...
  imageOffsetX?: number; // X offset for image inside frame (-100 to 100)
  imageOffsetY?: number; // Y offset for image inside frame (-100 to 100)
}
...
export const PhotoFieldEditor: React.FC<PhotoFieldProps> = ({
  ...
  return (
    <>
      ...
      <div className="border border-gray-200 rounded-lg p-3 mb-3 bg-gray-50">
        <label className="block text-gray-700 text-xs font-medium mb-2">Image Offset (Position inside frame)</label>
        <div className="space-y-3">
          <div>
            <label className="block text-gray-500 text-[10px] uppercase mb-1">X Offset: {field.imageOffsetX || 0}%</label>
            <div className="flex items-center gap-2">
              <input
                type="range"
                min="-100"
                ...
                onChange={(e) => onUpdate({ ...field, imageOffsetX: parseInt(e.target.value) })}
                className="flex-1 h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-purple-600"
              />
              <button 
                onClick={() => onUpdate({ ...field, imageOffsetX: 0 })}
          ...
          <div>
            <label className="block text-gray-500 text-[10px] uppercase mb-1">Y Offset: {field.imageOffsetY || 0}%</label>
            <div className="flex items-center gap-2">
              <input
                type="range"
                min="-100"
                max="100"
                step="1"
                value={field.imageOffsetY || 0}
                onChange={(e) => onUpdate({ ...field, imageOffsetY: parseInt(e.target.value) })}
                className="flex-1 h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-purple-600"
              />
              <button 
                onClick={() => onUpdate({ ...field, imageOffsetY: 0 })}
                className="text-[10px] text-purple-600 hover:text-purple-800"
              >
                Reset
              </button>
...
// Draw photo on canvas with proportional fitting (object-fit: cover style) and offsets
export const drawPhotoOnCanvas = async (
  ...
  try {
    ...
    if (imgAspect > fieldAspect) {
      ...
      baseDrawX = field.x;
      baseDrawY = field.y - (drawHeight - field.height) / 2;
    }

    // Apply offsets
    // Offsets are percentages of the extra space available
    const extraWidth = Math.max(0, drawWidth - field.width);
    const extraHeight = Math.max(0, drawHeight - field.height);
    
    const offsetX = ((field.imageOffsetX || 0) / 100) * (extraWidth / 2);
    const offsetY = ((field.imageOffsetY || 0) / 100) * (extraHeight / 2);

    ctx.drawImage(img, baseDrawX + offsetX, baseDrawY + offsetY, drawWidth, drawHeight);
    ctx.restore();
  } catch (error) {
