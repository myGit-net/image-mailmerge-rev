          ...
          <div key={index} className={`p-3 rounded-lg border ${selectedFieldIndex === index ? 'border-blue-500 bg-blue-50' : 'border-gray-300 bg-gray-50'}`}>
            ...
            {field.type !== 'photo' && (
              ...
              />
	            ...
	            {field.type === 'text' ? (
	              <>
	                ...
	                <div className="flex gap-1 mb-2">
	                  <button
	                    type="button"
	                    onClick={() => onFieldUpdate(index, { ...field, isBold: !field.isBold })}
	                    className={`w-8 h-8 rounded border flex items-center justify-center font-bold transition-colors ${field.isBold ? 'bg-blue-600 text-white border-blue-700' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-100'}`}
	                    title="Bold"
	                  >
	                    B
	                  </button>
	                  <button
	                    type="button"
	                    onClick={() => onFieldUpdate(index, { ...field, isItalic: !field.isItalic })}
	                    className={`w-8 h-8 rounded border flex items-center justify-center italic transition-colors ${field.isItalic ? 'bg-blue-600 text-white border-blue-700' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-100'}`}
	                    title="Italic"
	                  >
	                    I
	                  </button>
	                  <button
	                    type="button"
	                    onClick={() => onFieldUpdate(index, { ...field, isUnderline: !field.isUnderline })}
	                    className={`w-8 h-8 rounded border flex items-center justify-center underline transition-colors ${field.isUnderline ? 'bg-blue-600 text-white border-blue-700' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-100'}`}
	                    title="Underline"
	                  >
	                    U
	                  </button>
	                  <button
	                    type="button"
	                    onClick={() => onFieldUpdate(index, { ...field, isStrikethrough: !field.isStrikethrough })}
	                    className={`w-8 h-8 rounded border flex items-center justify-center line-through transition-colors ${field.isStrikethrough ? 'bg-blue-600 text-white border-blue-700' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-100'}`}
	                    title="Strikethrough"
	                  >
	                    S
	                  </button>
                ...
                <div className="border border-gray-200 rounded-lg p-3 mb-3 bg-gray-50">
                  <label className="block text-gray-700 text-xs font-medium mb-2">Stroke</label>
                  <div className="space-y-2">
                    <div className="flex items-center gap-2">
                      <label className="text-xs text-gray-600 w-12 flex-shrink-0">Width</label>
                      <input
                        type="range"
                        min="0"
                        max="20"
                        ...
                        onChange={(e) => onFieldUpdate(index, { ...field, strokeWidth: parseFloat(e.target.value) })}
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          if (!Number.isNaN(val) && val >= 0 && val <= 20) {
                            onFieldUpdate(index, { ...field, strokeWidth: val });
                          }
                        }}
                        className="bg-white border border-gray-300 text-gray-900 px-2 py-1 rounded text-sm w-16 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                      />
                    </div>
                    {field.strokeWidth > 0 && (
                      <div>
                        ...
                        <div className="flex flex-wrap gap-1 mb-1">
                          {COLOR_PRESETS.map(color => (
                            <button
                              ...
                              onClick={() => onFieldUpdate(index, { ...field, strokeColor: color.value })}
                              className={`w-6 h-6 rounded border-2 ${field.strokeColor === color.value ? 'border-gray-800' : 'border-gray-300'} hover:border-gray-800 transition-colors`}
                              style={{ backgroundColor: color.value }}
                              title={color.name}
                            />
                          ))}
                        ...
                        <details className="mt-1">
                          ...
                          <input
                            ...
                            onChange={(e) => onFieldUpdate(index, { ...field, strokeColor: e.target.value })}
                            className="w-full h-8 bg-white border border-gray-300 rounded cursor-pointer mt-1"
                          />
                        </details>
                      </div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
	                ...
	                <div className="border border-gray-200 rounded-lg p-3 bg-gray-50">
	                  <div className="flex items-center justify-between mb-2">
	                    <label className="text-gray-700 text-xs font-medium">Text Wrapping</label>
	                    <label className="relative inline-flex items-center cursor-pointer">
	                      <input 
	                        type="checkbox" 
	                        checked={field.wrapText} 
	                        onChange={(e) => onFieldUpdate(index, { ...field, wrapText: e.target.checked })}
		                  ...
		                  {field.wrapText && (
		                    <div className="space-y-2 mt-2">
		                      <div className="flex items-center gap-2">
		                        <label className="text-xs text-gray-600 w-16 flex-shrink-0">Max Width</label>
		                        <input
		                          type="range"
		                          min="10"
		                          max="2000"
		                          step="1"
		                          value={field.maxWidth || 200}
		                          onChange={(e) => onFieldUpdate(index, { ...field, maxWidth: parseInt(e.target.value) })}
		                          className="flex-1 h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-blue-600"
		                        />
		                        <input
		                          type="number"
		                          ...
		                          onChange={(e) => {
		                            ...
		                            if (!Number.isNaN(val)) {
		                              onFieldUpdate(index, { ...field, maxWidth: val });
		                            }
		                          }}
		                          className="bg-white border border-gray-300 text-gray-900 px-2 py-1 rounded text-sm w-16 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
		                        />
