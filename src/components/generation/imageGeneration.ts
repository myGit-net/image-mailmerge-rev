...
import { drawPhotoOnCanvas } from '../PhotoField';
  ...
  try {
    ...
    for (let i = 0; i < totalRows; i++) {
      ...
      for (const field of fields) {
        ...
        if (field.type === 'photo') {
          // For photo fields: use CSV column value if mapped, otherwise use the locally uploaded demo image
          const csvValue = mapping?.csvColumn ? row[mapping.csvColumn] || '' : '';
          const photoSrc = csvValue.trim() || field.demoImageDataUrl || '';
          if (photoSrc) {
