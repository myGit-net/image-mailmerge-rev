...
import { PhotoFieldData } from '../PhotoField';
...
export const getFieldDisplayText = (
  ...
  // For photo fields, return the demoImageDataUrl as the display value
  // (the actual image data URL for rendering on canvas)
  if (field.type === 'photo') {
    if (useActualData && csvData) {
      ...
      if (rowData) {
        ...
        if (mapping?.csvColumn) {
          ...
          if (csvValue.trim()) return csvValue;
        }
      }
    }
