...
import { PhotoFieldData, drawPhotoOnCanvas, resolvePhotoSource } from './components/PhotoField';
import { parseMarkdownText, applyTextFormatting, drawFormattedText, getTextBounds } from './components/text/textFormatting';
...
import { generateImages as generateImagesUtil, preloadFonts } from './components/generation/imageGeneration';
...
const ImageMailMerge: React.FC = () => {
  ...
  const [pendingFieldType, setPendingFieldType] = useState<'text' | 'qrcode' | 'photo' | null>(null);
  const [newFieldName, setNewFieldName] = useState('');
  const [decrementCredits, setDecrementCredits] = useState<DecrementCreditsFn | null>(null);
  const [isExporting, setIsExporting] = useState(false);
  const [isImporting, setIsImporting] = useState(false);
  const [photoLibrary, setPhotoLibrary] = useState<Map<string, string>>(new Map());
  ...
  const handleFieldTypeSelection = useCallback((fieldType: 'text' | 'qrcode' | 'photo') => {
  ...
  const handleCreateField = useCallback(() => {
    ...
    } else if (pendingFieldType === 'qrcode') {
      const field = createQRField(trimmedName, ui.pendingFieldPosition.x, ui.pendingFieldPosition.y);
      addFieldToList(field, fieldState.setFields, fieldState.setFieldMappings);
    } else if (pendingFieldType === 'photo') {
      const field = createPhotoField(trimmedName, ui.pendingFieldPosition.x, ui.pendingFieldPosition.y);
      addFieldToList(field, fieldState.setFields, fieldState.setFieldMappings);
    }
  ...
  const drawFields = useCallback(async () => {
    ...
    for (let index = 0; index < fieldState.fields.length; index++) {
      ...
      if (field.type === 'text') {
        ...
        // For photo fields, get the image data URL (local upload or resolved from photo library)
        const photoImageSrc = getFieldDisplayText(field, spreadsheet.csvData, spreadsheet.currentCsvRowIndex, fieldState.fieldMappings, true, photoLibrary);
        
        if (photoImageSrc && photoImageSrc.trim()) {
          await drawPhotoOnCanvas(ctx, field, photoImageSrc);
        } else {
          // Draw placeholder for photo field
          ctx.save();
          ...
          // Draw a small camera/photo icon
          const iconSize = Math.min(field.width, field.height, 40) * 0.5;
          ctx.strokeStyle = '#7c3aed';
          ctx.lineWidth = 1.5;
          ctx.strokeRect(centerX - iconSize, centerY - iconSize * 0.7, iconSize * 2, iconSize * 1.4);
          // Mountain icon inside
          ctx.beginPath();
          ctx.moveTo(centerX - iconSize * 0.7, centerY + iconSize * 0.4);
          ctx.lineTo(centerX - iconSize * 0.2, centerY - iconSize * 0.1);
          ctx.lineTo(centerX + iconSize * 0.2, centerY + iconSize * 0.2);
          ctx.lineTo(centerX + iconSize * 0.5, centerY - iconSize * 0.2);
          ctx.lineTo(centerX + iconSize * 0.7, centerY + iconSize * 0.4);
          ctx.closePath();
          ...
          ctx.textAlign = 'left';
          ctx.textBaseline = 'middle';
          ctx.fillText(`${index + 1}`, field.x + field.width + 8, field.y + 8);
          
          ctx.font = '12px Arial';
          ctx.fillText(field.name, field.x + field.width + 8, field.y + 24);
        }
  ...
  }, [fieldState.fields, fieldState.selectedFieldIndex, spreadsheet.csvData, spreadsheet.currentCsvRowIndex, fieldState.fieldMappings, canvas, photoLibrary]);
  ...
  const getFieldAtPosition = useCallback((x: number, y: number): { index: number, isResizeHandle: boolean } => {
    ...
    for (let i = fieldState.fields.length - 1; i >= 0; i--) {
      ...
      const displayText = getFieldDisplayText(field, spreadsheet.csvData, spreadsheet.currentCsvRowIndex, fieldState.fieldMappings);
        ...
        // For circle frame, use circular hit test
        if (field.frame === 'circle') {
          const diameter = Math.min(field.width, field.height);
          const centerX = field.x + field.width / 2;
          const centerY = field.y + field.height / 2;
  ...
  const handleMouseDown = useCallback((e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (hit.index >= 0) {
      ...
      if (hit.isResizeHandle) {
        ...
        fieldState.dragStartRef.current.fontSize = field.type === 'text' ? field.fontSize : 
          field.type === 'qrcode' ? field.size : 
          field.type === 'photo' ? field.width : 0;
  ...
  const handleWheelNative = useCallback((e: WheelEvent) => {
    ...
    if (hit.index >= 0) {
      ...
      if (field.type === 'text') {
        ...
        const scaleDelta = delta * 2;
        const aspectRatio = field.width / field.height;
        const newWidth = Math.max(10, field.width + scaleDelta);
        const newHeight = Math.max(10, Math.round(newWidth / aspectRatio));
        fieldState.setFields(prev => prev.map((f, i) => 
          i === hit.index ? { ...f, width: newWidth, height: newHeight } : f
        ));
      }
    }
  ...
  }, [canvas.templateImage, fieldState.fields, fieldState.fieldMappings, spreadsheet.fileNameMapping, spreadsheet.csvData, spreadsheet.csvHeaders, photoLibrary]);
  ...
  const handleImportProject = useCallback(async (file: File) => {
    ...
    try {
      ...
      // Preload fonts for the imported fields to ensure preview is correct
      preloadFonts(result.fields).then(() => {
        drawFields();
      });
      ...
      setPhotoLibrary(result.photoLibrary);
  ...
  }, [canvas.templateImage, spreadsheet.csvData, fieldState.fields, fieldState.fieldMappings, spreadsheet.fileNameMapping, canvas.imageUrl, ui, decrementCredits, photoLibrary]);

  const isReadyToGenerate = checkReadyToGenerate(canvas.templateImage, spreadsheet.csvData, fieldState.fields);

  // Preload fonts whenever fields change to ensure preview is accurate
  useEffect(() => {
    if (fieldState.fields.length > 0) {
      preloadFonts(fieldState.fields).then(() => {
        drawFields();
      });
    }
  }, [fieldState.fields, drawFields]);
    ...
    <>
      ...
      <div ref={ui.mainLayoutRef} className="flex h-full min-h-0 bg-white">
        ...
        <Sidebar
          ...
          onPhotoLibraryChange={setPhotoLibrary}
        ...
        <FieldTypeModal
          ...
          onPhotoFieldSelect={() => handleFieldTypeSelection('photo')}
          onCancel={() => {
            ui.setShowFieldTypeModal(false);
